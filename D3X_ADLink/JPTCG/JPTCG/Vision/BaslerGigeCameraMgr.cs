...
namespace JPTCG.Vision
{
    class BaslerGigeCameraMgr
    {
        ...
        public static BaslerGigeCameraMgr mCGigecamera;

        public static BaslerGigeCameraMgr Inistance()
        {
            if (mCGigecamera == null) mCGigecamera = new BaslerGigeCameraMgr();
            return mCGigecamera;
        }


        public BaslerGigeCameraMgr()
        {
            ...
            try
            {
                while (m_grabThreadRun)
                {
                    ...
                    if (grabResult.Status == EPylonGrabStatus.Grabbed)
                    {
                        ...
                        {
                            mhalcon_image2.GenEmptyObj();
                            if (BaslerGigeCameraMgr.IsCompatible(m_bitmap, image.Width, image.Height, image.Color))
                            {
